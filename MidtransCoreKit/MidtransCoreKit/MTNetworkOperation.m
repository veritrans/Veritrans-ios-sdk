...
@interface MTNetworkOperation() <NSURLConnectionDataDelegate, NSURLConnectionDelegate>
@property (nonatomic, strong) NSURLConnection *connection;
@property (nonatomic, copy) void (^callback)(id response, NSError *error);
@end

@implementation MTNetworkOperation {
    BOOL _isFinished;
...
+ (instancetype)operationWithRequest:(NSURLRequest *)request callback:(void(^)(id response, NSError *error))callback {
    MTNetworkOperation *op = [[MTNetworkOperation alloc] init];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    if (responseObject[MT_CORE_STATUS_CODE]) {
        NSInteger code = [responseObject[MT_CORE_STATUS_CODE] integerValue];
        if (code/100 == 2) {
            if (self.callback) self.callback(responseObject, nil);
        } else {
            NSError *error = [NSError errorWithDomain:MT_ERROR_DOMAIN
                                                 code:code
