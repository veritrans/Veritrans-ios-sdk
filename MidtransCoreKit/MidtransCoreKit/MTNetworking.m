...
+ (id)sharedInstance {
    static MTNetworking *shared = nil;
             ...
             callback:(void(^)(id response, NSError *error))callback
{
    ...
    MTNetworkOperation *op = [MTNetworkOperation operationWithRequest:request
                                                             callback:callback];
...
+ (void)handleResponse:(id)response callback:(void(^)(id response, NSError *error))callback {
    ...
    NSInteger code = [response[MT_CORE_STATUS_CODE] integerValue];
    NSString *message = response[@"status_message"];
    
    if (code == 200) {
        _response = response;
    } else {
        error = [[NSError alloc] initWithDomain:MT_ERROR_DOMAIN code:code userInfo:@{NSLocalizedDescriptionKey:message}];
    }
