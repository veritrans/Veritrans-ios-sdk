...
- (void)performTransaction:(MidtransTransaction *)transaction completion:(void(^)(MidtransTransactionResult *result, NSError *error))completion {
    ...
    [[MidtransNetworking sharedInstance] postToURL:URL header:headers parameters:[transaction dictionaryValue] callback:^(id response, NSError *error) {
        ...
        if (response) {
            ...
            if ([paymentType isEqualToString:VT_PAYMENT_CREDIT_CARD]) {
                [[MidtransTrackingManager sharedInstance]trackTransaction:YES secureProtocol:YES withPaymentFeature:0 paymentMethod:VT_PAYMENT_CREDIT_CARD value:0];
...
- (void)fetchMaskedCardsCustomer:(MidtransCustomerDetails *)customer completion:(void(^_Nullable)(NSArray *_Nullable maskedCards, NSError *_Nullable error))completion{
...
- (BOOL)isWebPaymentType:(NSString *)paymentType {
    ...
    [paymentType isEqualToString:VT_PAYMENT_KLIK_BCA];
}
