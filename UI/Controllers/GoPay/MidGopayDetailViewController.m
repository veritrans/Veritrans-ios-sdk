...
- (void)viewDidLoad {
    ...
    if (IPAD) {
        ...
        NSDate *endDate = [expireFormatter dateFromString: [[self.paymentResult.expiration stringByReplacingOccurrencesOfString:@"WIB" withString:@""] stringByAppendingString:currentYearString]];
        self.deltaTime = [endDate timeIntervalSinceDate:self.paymentResult.transactionTime];
...
- (void)processCheckOut {
    if (IPAD) {
        ...
        NSDictionary *userInfo = @{TRANSACTION_RESULT_KEY:self.paymentResult.dictionaryValue};
        [[NSNotificationCenter defaultCenter] postNotificationName:TRANSACTION_PENDING object:nil userInfo:userInfo];
        NSURL *gojekConstructURL = [NSURL URLWithString:self.paymentResult.deepLinkURL];
